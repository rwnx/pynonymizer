...
- Added Locale key `locale`(top-level) to strategyfiles. This is designed to replace the `-l`/`--locale` CLI option. 
- Deprecated `-l`/`--locale` option. You should use the `locale` key in your strategyfile. 
