...
def test_empty_where():
    empty_where = EmptyUpdateColumnStrategy(
        column_name="empty_column", where="`cheese` = 'gouda'"
    )
...
def test_ulogin_where():
    ulogin_where = UniqueLoginUpdateColumnStrategy(
        column_name="column_name", where="`cheese` = 'edam'"
    )
...
def test_uemail_where():
    uemail = UniqueEmailUpdateColumnStrategy(
        column_name="column_name", where="`cheese` = 'gouda'"
    )
...
def test_literal_where():
    literal_where = LiteralUpdateColumnStrategy(
        column_name="column_name", value="RAND()", where="cake = 'death' OR gravel > 3"
    )
...
def test_fake_update_sql_type():
    fake_column_generator = Mock()
    fake_update_sql_type = FakeUpdateColumnStrategy(
        column_name="column_name",
        fake_column_generator=fake_column_generator,
        fake_type="company_email",
        sql_type="UUID",
    ...
    assert fake_update_sql_type.value == fake_column_generator.get_value(
        "company_email"
    )
    assert fake_update_sql_type.data_type == fake_column_generator.get_data_type(
        "company_email"
    )
