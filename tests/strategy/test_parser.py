from pynonymizer.fake import UnsupportedFakeArgumentsError, UnsupportedFakeTypeError, FakeColumnGenerator
...
def test_custom_provider_missing_import_should_raise_error(strategy_parser, invalid_custom_provider_unknown_module):
...
def test_smoke_test__should_contain_truncate_table(strategy_parser, smoke_test):
    ...
    truncate = next( x for x in strategy.table_strategies if x.table_name == "truncate_table" )
...
def test_smoke_test__should_contain_delete_table(strategy_parser, smoke_test):
    ...
    delete = next( x for x in strategy.table_strategies if x.table_name == "delete_table" )
...
def test_smoke_test__should_contain_update_column_table(strategy_parser, smoke_test):
    strategy = strategy_parser.parse_config(smoke_test)
    update_columns = next( x for x in strategy.table_strategies if x.table_name == "update_column_table" )
...
def test_smoke_test__update_columns_should_parse_verbose_and_shorthand_identical(strategy_parser, smoke_test):
    strategy = strategy_parser.parse_config(smoke_test)

    update_columns = next( x for x in strategy.table_strategies if x.table_name == "update_column_table" )

    unique_login = next( x for x in update_columns.column_strategies if x.column_name == "unique_login" )
    unique_login_verbose = next( x for x in update_columns.column_strategies if x.column_name == "unique_login_verbose" )

    unique_email = next( x for x in update_columns.column_strategies if x.column_name == "unique_email" )
    unique_email_verbose = next( x for x in update_columns.column_strategies if x.column_name == "unique_email_verbose" )

    fake_update_shorthand = next( x for x in update_columns.column_strategies if x.column_name == "fake_update_shorthand" )
    fake_update_verbose = next( x for x in update_columns.column_strategies if x.column_name == "fake_update_verbose" )

    literal_column_shorthand = next( x for x in update_columns.column_strategies if x.column_name == "literal_column_shorthand" )
    literal_column_verbose = next( x for x in update_columns.column_strategies if x.column_name == "literal_column_verbose" )
    ...
    assert literal_column_shorthand.strategy_type == literal_column_verbose.strategy_type
    assert literal_column_shorthand.value == literal_column_verbose.value



def test_unsupported_fake_column_type__should_raise_unsupported_fake_type(strategy_parser, invalid_unsupported_fake_type):
    with pytest.raises(UnsupportedFakeTypeError):
        strategy_parser.parse_config(invalid_unsupported_fake_type)

def test_unsupported_fake_column_type__should_raise_unsupported_fake_args(strategy_parser, invalid_unsupported_fake_args):
    with pytest.raises(UnsupportedFakeArgumentsError):
        strategy_parser.parse_config(invalid_unsupported_fake_args)

def test_unsupported_fake_column_type__should_raise_unsupported_fake_args(strategy_parser, invalid_unsupported_table_type):
    with pytest.raises(UnknownTableStrategyError):
        strategy_parser.parse_config(invalid_unsupported_table_type)

def test_unsupported_fake_column_type__should_raise_unsupported_fake_args(strategy_parser, invalid_unsupported_column_type):
