...
def test_init_runners_correctly(execution):
    ...
    execution.MySqlCmdRunner.assert_called_once_with(
        db_host="1.2.3.4",
        db_user="root",
        db_pass="password",
        db_name="db_name",
        db_port="3306",
        additional_opts="--test-arg=cmd",
    )
    execution.MySqlDumpRunner.assert_called_once_with(
        db_host="1.2.3.4",
        db_user="root",
        db_pass="password",
        db_name="db_name",
        db_port="3306",
        additional_opts="--test-arg=3",
    )
...
def test_create_database(query_factory, execution):
    ...
    execution.MySqlCmdRunner.return_value.execute.assert_called_once_with(
...
def test_dump_database(resolve_output, query_factory, execution):
    ...
    execution.MySqlDumpRunner.return_value.open_dumper.return_value = MagicMock(
...
def test_after_script_run(query_factory, execution):
    ...
    after_script_call_index = manager.mock_calls.index(
        call.execution.MySqlCmdRunner().db_execute("SELECT `after` FROM `after_table`;")
    )
...
def test_anonymize_database(
    ...
    query_factory.get_insert_seed_row.assert_has_calls(
        ([call(*seed_table_qualifier_map)] * 150)
    )
    ...
    ix_create_seed = manager.mock_calls.index(
        call.execution.MySqlCmdRunner().db_execute(
            query_factory.get_create_seed_table()
        )
    )
    ix_insert_seed_first = manager.mock_calls.index(
        call.execution.MySqlCmdRunner().db_execute(query_factory.get_insert_seed_row())
    )
    ...
    ix_trunc_table = manager.mock_calls.index(
        call.execution.MySqlCmdRunner().db_execute(query_factory.get_truncate_table())
    )
    ix_update_table = manager.mock_calls.index(
        call.execution.MySqlCmdRunner().db_execute(query_factory.get_update_table())
    )
    ix_delete_table = manager.mock_calls.index(
        call.execution.MySqlCmdRunner().db_execute(query_factory.get_delete_table())
    )
    ix_drop_seed = manager.mock_calls.index(
        call.execution.MySqlCmdRunner().db_execute(query_factory.get_drop_seed_table())
    )
