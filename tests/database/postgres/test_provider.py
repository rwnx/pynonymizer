...
def test_init_runners_correctly(execution):
    ...
    PostgreSqlProvider(
        ...
        seed_rows=150,
        cmd_opts="--quick",
        dump_opts="--additional",
    )
    execution.PSqlCmdRunner.assert_called_once_with(
        db_host="1.2.3.4",
        db_user="root",
        db_pass="password",
        db_name="db_name",
        db_port="5432",
        additional_opts="--quick",
    )
    execution.PSqlDumpRunner.assert_called_once_with(
        db_host="1.2.3.4",
        db_user="root",
        db_pass="password",
        db_name="db_name",
        db_port="5432",
        additional_opts="--additional",
    )
...
def test_create_database(query_factory, execution):
    provider = PostgreSqlProvider("1.2.3.4", "root", "password", "db_name", seed_rows=150)
    ...
    execution.PSqlCmdRunner.return_value.execute.assert_called_once_with(
        query_factory.get_create_database.return_value
    )
...
def test_drop_database(query_factory, execution):
    ...
    execution.PSqlCmdRunner.return_value.execute.assert_called_once_with(
        query_factory.get_drop_database.return_value
    )
...
def test_restore_database(resolve_input, query_factory, execution):
    ...
    execution.PSqlCmdRunner.return_value.open_batch_processor.assert_called_once_with()
    execution.PSqlCmdRunner.return_value.open_batch_processor.return_value.write.assert_called()
    execution.PSqlCmdRunner.return_value.open_batch_processor.return_value.flush.assert_called()


@patch("pynonymizer.database.postgres.execution", autospec=True)
@patch("pynonymizer.database.postgres.query_factory", autospec=True)
@patch("pynonymizer.database.postgres.resolve_output")
def test_dump_database(resolve_output, query_factory, execution):
    ...
    execution.PSqlDumpRunner.return_value.open_dumper.return_value = MagicMock(
        read=MagicMock(side_effect=[rand_data, b""])
    )
    ...
    execution.PSqlDumpRunner.return_value.open_dumper.assert_called_once_with()
    execution.PSqlDumpRunner.return_value.open_dumper.return_value.read.assert_called()
    ...
    with pytest.raises(Exception) as e_info:
        ...
        database_strategy = DatabaseStrategy(
            [
                Mock(
                    table_name="table1",
                    strategy_type="DEFINITELY_NOT_A_SUPPORTED_STRATEGY_TYPE",
                ),
            ]
        )
...
def test_after_script_run(query_factory, execution):
    ...
    after_script_call_index = manager.mock_calls.index(
        call.execution.PSqlCmdRunner().db_execute("SELECT `after` FROM `after_table`;")
    )
...
def test_anonymize_database(
    ...
    provider = PostgreSqlProvider(
        "1.2.3.4", "root", "password", "db_name", seed_rows=200
    )

    provider.anonymize_database(simple_strategy)

    seed_table_qualifier_map = [
        "_pynonymizer_seed_fake_data",
        {"user_name": simple_strategy_update_fake_column},
    ]

    # get_create_seed_table( seed table name, qualifier map)
    query_factory.get_create_seed_table.assert_called_once_with(
        *seed_table_qualifier_map
    )

    # seed table should be called with the qual map 150 times
    query_factory.get_insert_seed_row.assert_has_calls(
        ([call(*seed_table_qualifier_map)] * 200)
    )
    ...
    query_factory.get_update_table.assert_called_once_with(
        "_pynonymizer_seed_fake_data", simple_strategy_update
    )
    query_factory.get_drop_seed_table.assert_called_once_with(
        "_pynonymizer_seed_fake_data"
    )

    # anonymize basic order checks
    ix_create_seed = manager.mock_calls.index(
        call.execution.PSqlCmdRunner().db_execute(query_factory.get_create_seed_table())
    )
    ix_insert_seed_first = manager.mock_calls.index(
        call.execution.PSqlCmdRunner().db_execute(query_factory.get_insert_seed_row())
    )
    ix_insert_seed_last = list_rindex(
        manager.mock_calls,
        call.execution.PSqlCmdRunner().db_execute(query_factory.get_insert_seed_row()),
    )
    ix_trunc_table = manager.mock_calls.index(
        call.execution.PSqlCmdRunner().db_execute(query_factory.get_truncate_table())
    )
    ix_update_table = manager.mock_calls.index(
        call.execution.PSqlCmdRunner().db_execute(query_factory.get_update_table())
    )
    ix_drop_seed = manager.mock_calls.index(
        call.execution.PSqlCmdRunner().db_execute(query_factory.get_drop_seed_table())
    )
