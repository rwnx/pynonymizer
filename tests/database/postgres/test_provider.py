...
def test_init_runners_correctly(execution):
    ...
    """
    PostgreSqlProvider("1.2.3.4", "root", "password", "db_name", cmd_opts="--quick", dump_opts="--additional")
    execution.PSqlCmdRunner.assert_called_once_with(db_host="1.2.3.4", db_user="root", db_pass="password", db_name="db_name", db_port="5432", additional_opts="--quick")
    execution.PSqlDumpRunner.assert_called_once_with(db_host="1.2.3.4", db_user="root", db_pass="password", db_name="db_name", db_port="5432", additional_opts="--additional")
...
def test_create_database(query_factory, execution):
    ...
    execution.PSqlCmdRunner.return_value.execute.assert_called_once_with(query_factory.get_create_database.return_value)
...
def test_drop_database(query_factory, execution):
    ...
    execution.PSqlCmdRunner.return_value.execute.assert_called_once_with(query_factory.get_drop_database.return_value)
...
def test_dump_database(resolve_output, query_factory, execution):
    ...
    execution.PSqlDumpRunner.return_value.open_dumper.return_value = MagicMock(read=MagicMock(side_effect=[rand_data, b""]))
...
def test_before_script_run(query_factory, execution):
    ...
    before_script_call_index = manager.mock_calls.index( call.execution.PSqlCmdRunner().db_execute("SELECT `before` FROM `before_table`;") )
